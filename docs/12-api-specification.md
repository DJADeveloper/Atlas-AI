...
1. **OpenAPI-first.** The FastAPI app (Pydantic v2 models) is the single source of
   the OpenAPI document. `packages/api-client` is **generated** from that document
   and is the *only* way the UI talks to the API — no hand-rolled `fetch` calls.
   Contract drift becomes a TypeScript compile error, not a runtime surprise.
2. **Localhost-only.** The API binds to `127.0.0.1`. There is no remote surface in
   local mode; auth exists to stop *other local processes*, not the internet.
3. **Traceable by construction.** Every response — success or failure, JSON or
   SSE — carries `X-Trace-Id`. Error bodies repeat it as `trace_id` so a user can
   paste one string into a bug report and we can replay the whole request.
...
5. **Uniform conventions.** Section 3 applies to *every* endpoint. Endpoint
   sections document only what deviates.
6. **Read-only by default.** The mutating surface is deliberately small; every
   mutation is idempotency-guarded (§3.7) and lands in the audit trail when it is
   security-relevant.

## 2. Authentication

Local token auth (spine §5, `atlas/auth`), designed so JWT/OIDC can slot in later
without changing routes.

- On first run the Tauri shell generates a **device secret**, stores it in the OS
  keychain, and registers its hash in `api_keys`. Browser-based dev uses a
  pairing code printed by `atlas dev`.
- `POST /api/v1/auth/token` exchanges the device secret for a short-lived opaque
...
|---|---|---|
| `401 Unauthorized` | *Who are you?* Missing, expired, or invalid token. Always includes `WWW-Authenticate: Bearer`. Client should re-run the token exchange. | `authentication_required`, `invalid_token` |
| `403 Forbidden` | *I know who you are; the answer is no.* Capability/permission denial by the policy engine, or a feature flag is off. Re-authenticating will not help. | `permission_denied`, `feature_disabled` |

## 3. Conventions

These apply globally; they are stated once here and never repeated per endpoint.

### 3.1 JSON casing — `snake_case`

All request and response fields are `snake_case`. Rationale: the domain is
Python/Pydantic and the database is snake_case (spine §7); keeping **one
canonical name per field** across DB column, domain model, API payload, log line,
and generated TS client eliminates an entire class of mapping bugs and makes
`grep fused_score` work across the whole monorepo. The alternative — camelCase
at the edge with a translation layer — buys JavaScript aesthetics at the price of
two names for everything and a serializer that can silently drift. The generated
client preserves snake_case; the UI adapts, not the contract.
...
`019f8a3c-6b21-7d4e-8a2f-3c9d1e5b7a01`. Time-ordered, so ids sort by creation
and make good cursor components.

### 3.4 Pagination — opaque cursor

Every list endpoint accepts `?limit=` (default 20, max 100) and `?cursor=`, and
returns:
...
`next_cursor` is `null` when the page is the last one. The cursor is an opaque
base64url token encoding the sort-key values and id of the last row; clients
must treat it as a black box.

**Why cursor, not offset.** Offset pagination (`?page=3`) is O(n) — Postgres
must scan and discard every skipped row — and it is *unstable*: an insert between
page fetches shifts every subsequent page, so clients see duplicates or gaps.
A keyset cursor is an index seek (O(log n)) and pins the page boundary to a
concrete row, so concurrent writes (a background indexer is *always* writing in
Atlas) cannot corrupt iteration. The trade-off — no "jump to page 7" — is one no
Atlas screen needs; every list is an infinite scroll or a filtered drill-down.

### 3.5 Filtering

- Simple filters are query parameters on GET lists: `?state=running`,
...
- Structured, multi-field filters (search) go in a POST body — they outgrow
  query strings.
- Default sort is newest-first by `id` (UUIDv7 ⇒ creation order); endpoints
  offering alternatives document a `?sort=` enum.

### 3.6 Error model — RFC 9457 + stable code registry

Content type `application/problem+json`. Shape (always):
...
follow-up ids (approval id, retry-after, budget numbers). The `code` registry is
**append-only** — codes are never renamed or reused:

| `code` | HTTP | Meaning |
|---|---|---|
| `validation_error` | 422 | Request shape/values failed Pydantic validation; see `errors[]`. |
...
| `conflict` | 409 | State conflict: duplicate source path, PATCH against stale `updated_at`, etc. |
| `idempotency_key_reuse` | 409 | Same `Idempotency-Key` replayed with a *different* payload. |
| `stream_in_progress` | 409 | Conversation already has an active generation; wait or cancel it. |
| `index_stale` | 409 | `require_fresh` search while ingestion jobs are pending for the filtered scope. |
| `agent_run_terminal` | 409 | Cancel/resume on a run already in a terminal state. |
| `approval_required` | 409 | Synchronous tool invocation parked pending human approval; `context.approval_id`. |
| `approval_expired` | 410 | Decision posted after the approval TTL; the invocation was auto-canceled. |
| `payload_too_large` | 413 | Body exceeds limit (1 MiB JSON). |
| `unsupported_file_type` | 415 | Source contains a file type no parser handles (surfaced on jobs, uploads). |
...
| `provider_timeout` | 504 | Provider exceeded its deadline and the request was abandoned. |
...
- Same key + same payload ⇒ the stored response is replayed (same status, body,
  and `Idempotent-Replayed: true` header). A retried "send message" does not
  create a second turn; a retried "approve" does not double-execute a move.
- Same key + different payload ⇒ `409 idempotency_key_reuse`.

This is the standard defense against the two-generals problem of "did my POST
land before the connection dropped?" — the client's retry loop becomes safe by
construction instead of by luck. PATCH/DELETE are naturally idempotent and do
not use the header.
...
| `Last-Event-ID` | request | SSE resume (§4.3.4). |
| `X-Trace-Id` | response | Always. W3C trace id, 32 hex chars. |
| `RateLimit-Limit` / `RateLimit-Remaining` / `RateLimit-Reset` | response | §6. |
...
Grouped exactly by spine §8. Paths below omit the `/api/v1` prefix.
...
| `GET /ready` | Readiness — Postgres reachable, migrations current, Redis reachable, Ollama optional. | 200 or 503 with per-dependency detail. No auth. |
...
Covered in §2. Errors: `invalid_token` 401 on bad secret; `rate_limited` 429 on
brute-force pacing (5 attempts/min).
...
```json
// 201
{ "id": "019f8a3c-6b21-7d4e-8a2f-3c9d1e5b7a01",
  "title": null, "project_id": "019f7c22-1d3e-7f4a-9b5c-6d7e8f9a0b1c",
  "message_count": 0, "created_at": "2026-07-20T14:30:11.021Z",
  "updated_at": "2026-07-20T14:30:11.021Z" }
```

`title` is auto-generated after the first turn by `claude-haiku-4-5-20251001`
(local profile: `llama3.1:8b`). Status 201; errors: `not_found` (project),
`validation_error`.

#### `GET /conversations` — list (cursor-paginated)

Filters: `?project_id=`, `?updated_after=`. Returns conversation summaries
(id, title, project_id, message_count, last_message_at).

#### `GET /conversations/{id}` — fetch metadata

404 `not_found` if absent. Messages are fetched separately:

#### `GET /conversations/{id}/messages` — list persisted messages

Cursor-paginated, oldest-first within the page window. Each item:

```json
{ "id": "019f8a3c-72b8-7f60-8c4b-5e1f3a7d9c23",
  "role": "assistant", "content": "You anchored Pro at $12/mo [1] …",
  "abstained": false,
  "citations": [ { "marker": 1,
...
Response: `200` with `Content-Type: text/event-stream` (see protocol below).
Pre-stream failures return normal problem+json: `budget_exceeded` 402 (caps
checked *before* the LLM call), `stream_in_progress` 409, `not_found` 404,
`validation_error` 422. Requires `Idempotency-Key` — a replayed key re-attaches
...
WebSocket buys bidirectionality Atlas's chat doesn't need, and costs a second
...
*reserved* for it. Note the generated client streams via `fetch` + a reader
(not `EventSource`, which cannot set the `Authorization` header).
...
integer `id` (the resume cursor). Every `data:` line is a single JSON object.
...
| `content_delta` | `{ "index": int, "delta": string }` | Many; `index` orders text fragments. |
| `citation` | `{ "marker": int, "chunk_id", "document_id", "document_title", "source_id", "snippet", "fused_score": float }` | As `[n]` markers are resolved; matches a persisted `citations` row. |
...
is retained for 15 minutes after `message_end`; after that the endpoint returns
404 `not_found` and the client falls back to
`GET /conversations/{id}/messages`, where the finished message is already
persisted. Nothing is ever lost — the buffer is a latency optimization over the
database, not the source of truth.
...
{ "query": "pro pricing decision",
  "filters": {
    "source_ids": ["019f89d1-2a4b-7c8d-9e0f-1a2b3c4d5e6f"],
...
Scores are all returned so the UI (and evals) can explain *why* a chunk ranked:
...
the cross-encoder (null when `rerank:false`). `abstain_recommended` mirrors the
chat abstention gate. `require_fresh: true` returns 409 `index_stale` when the
filtered scope has pending ingestion jobs. Errors: `validation_error`,
...
{ "id": "019f89d4-33c5-7a1b-8d2e-6f7a8b9c0d1e",
  "source_id": "019f89d1-2a4b-7c8d-9e0f-1a2b3c4d5e6f",
  "path": "notes/pricing-notes.md", "title": "pricing-notes.md",
  "file_type": "md",
...
```json
// 201
{ "id": "019f89d1-2a4b-7c8d-9e0f-1a2b3c4d5e6f",
  "kind": "folder", "name": "Project notes",
  "path": "/Users/darryl/Projects/atlas-notes",
  "state": "indexing", "paused": false,
  "document_count": 0,
...
| Endpoint | Purpose | Notes |
|---|---|---|
| `GET /sources` | List sources with `state`, `document_count`, `last_indexed_at`. | Filter `?state=`. |
...
| `DELETE /sources/{id}` | Unregister; enqueues async purge of documents/chunks. | 202 with `purge_job_id`. |
| `POST /sources/{id}/reindex` | Body `{ "force": false }`. `force:true` bypasses the hash gate. | 202 with `job_id`; `conflict` if a scan is already running. |

### 4.6 Jobs

| Endpoint | Purpose |
|---|---|
| `GET /jobs` | List ingestion jobs; filters `?source_id=`, `?state=` (`pending`/`running`/`succeeded`/`failed`/`skipped`), `?kind=` (`scan`/`ingest`/`reindex`/`purge`). |
| `GET /jobs/{id}` | Fetch one job with progress and error detail. |

```json
// GET /jobs/019f89d1-3b5c-7d6e-8f70-2a3b4c5d6e7f → 200
{ "id": "019f89d1-3b5c-7d6e-8f70-2a3b4c5d6e7f",
  "kind": "scan", "source_id": "019f89d1-2a4b-7c8d-9e0f-1a2b3c4d5e6f",
  "state": "running",
...
Failed jobs keep their last `error` (`{ "code": "unsupported_file_type",
...
```json
// 201
{ "id": "019f8b10-5c6d-7e8f-9a0b-1c2d3e4f5a6b",
  "kind": "preference",
  "content": "Prefers concise answers with bullet points over long prose",
...
with user confirmation (see [13-sequence-flows.md](13-sequence-flows.md), flow 8).
...
// 201 → { "id": "019f89c0-1122-7334-8556-778899aabbcc", "capability": "fs.read",
//          "scope": { "paths": ["/Users/darryl/Projects/atlas-notes/**"] },
...
after `settings.approvals.pending_ttl_minutes` (default 15); expiry auto-cancels
the invocation, writes an `audit_events` row, and a late decision returns 410
`approval_expired`. T3 approvals additionally require
`"confirmation_text": "DELETE"` matching the rendered preview's challenge.
Errors: `conflict` 409 (already decided), `not_found`.
...
// 201 → { "id": "019f8d77-0a1b-7c2d-8e3f-4a5b6c7d8e9f", "status": "queued",
//          "goal": "Organize my Downloads folder by file type",
...
List (filter `?status=`) and detail. Detail includes
...
{ "id": "019f8e88-1b2c-7d3e-9f4a-5b6c7d8e9f0a",
  "suite": "retrieval", "status": "succeeded",
  "dataset_id": "019f6e00-4a5b-7c6d-8e7f-9a0b1c2d3e4f", "example_count": 180,
  "metrics": { "recall_at_8": 0.91, "mrr_at_8": 0.74, "ndcg_at_8": 0.79,
               "abstention_accuracy": 0.88, "groundedness": 0.93 },
  "baseline_run_id": "019f8000-5b6c-7d7e-8f80-0a1b2c3d4e5f",
  "baseline_delta": { "recall_at_8": -0.006, "mrr_at_8": 0.011,
                      "ndcg_at_8": 0.004, "abstention_accuracy": 0.0,
                      "groundedness": -0.002 },
  "thresholds_passed": true, "cost_usd": 0.42,
  "git_sha": "b7e412c", "created_at": "2026-07-20T16:00:03.114Z",
  "finished_at": "2026-07-20T16:07:41.660Z" }
```

Per-example rows (`eval_results`) are queryable via
`GET /eval-runs/{id}?include=failures`, which appends the failing examples with
...
feature flag ships at M21; the protocol is specified with that milestone, not
here.
...
- Base path `/api/v1`. **Once the desktop app ships, `/api/v1` is frozen** in
  the n-1 sense: during auto-update the UI and the sidecar may skew by one
  release, so every `/v1` release must serve the previous release's client.
- **Additive changes are allowed** without a version bump: new endpoints, new
  ...
  `code`s, new enum values on fields documented as open. The corresponding
  client obligations are contractual: ignore unknown response fields, unknown
  SSE events, and unknown enum values on open enums.
...
- CI runs an OpenAPI diff against the last released spec and fails the PR on
  any non-additive change to `/v1` — the same gate pattern as the evals (flow 9).

## 6. Rate limiting

Token bucket per bearer token, kept in Redis. On localhost with one user this
is mostly a guard against runaway scripts and agent loops; the mechanism is the
same one a future multi-user deployment needs, so it ships now and the limits
simply tighten later.

| Bucket | Default |
|---|---|
| General requests | 120/min, burst 40 |
| LLM-backed requests (`POST …/messages`, `POST /agent-runs`, `POST /eval-runs`) | 20/min, burst 5 |
...
`rate_limited` with `Retry-After` seconds. Note that *cost* budgets
(`budget_exceeded`, 402) are a separate mechanism: rate limits protect the
process, budgets protect the wallet.

## 7. Decisions made in this document

Choices not already pinned by the spine, made here (simplest consistent option):
...
1. **Obvious sub-endpoints added:** `GET /conversations/{id}/messages`,
   ...
   `DELETE /projects/{id}/documents/{document_id}` — all consistent with §8's
   "representative" endpoint lists.
2. **Auth flow details:** device-secret-in-keychain → 24 h opaque bearer token via
   `POST /auth/token`; hashed secrets in `api_keys`.
3. **Error registry values:** the 20-code table in §3.6, including HTTP
   mappings (`budget_exceeded` → 402, `approval_required` → 409,
   `approval_expired` → 410, `index_stale` → 409).
...
5. **Pagination defaults:** limit default 20 / max 100; cursor = base64url
   keyset token; `next_cursor: null` terminates.
6. **SSE mechanics:** integer event ids per stream; Redis Stream buffer
   retained 15 min post-completion; `retry: 3000`; 15 s ping comments;
   generation detached from the connection.
...
9. **Search extras:** `require_fresh` flag backed by `index_stale`;
